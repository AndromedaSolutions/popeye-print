...
from weasyprint import HTML, LOGGER, VERSION, CSS
from weasyprint.text import cairo, pango
...
def main(argv=None, stdout=None, stdin=None):
    ...
    parser = argparse.ArgumentParser(
        prog='weasyprinter', description='Renders web pages to PDF or PNG.')
    ...
    parser.add_argument('-c', '--customfont', action='append',
                        help='URL or filename for a user custom font. '
                             'May be given multiple times.')
    ...
    for custom_font in args.customfont or []:
        ...
        custom_fonts.append(
            ...
            ''' % (based_name, custom_font)
        )
    custom_css_string = "\n".join(custom_fonts) if custom_fonts else None
    ...
    if custom_css_string:
        font_config = FontConfiguration()
        kwargs['stylesheets'] = kwargs.get('stylesheets') or []
        kwargs['stylesheets'].append(CSS(string=custom_css_string, font_config=font_config))
